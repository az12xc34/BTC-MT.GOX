...
import com.mt.gox.cn.adapter.ActionsAdapter;
import com.mt.gox.cn.fragment.BtcChinaFragment;
import com.mt.gox.cn.fragment.BtccDepthFetchFragment;
import com.mt.gox.cn.fragment.MtgoxDepthFetchFragment;
import com.mt.gox.cn.fragment.MtgoxWebFragment;
import com.mt.gox.cn.fragment.SandboxFragment;
import com.mt.gox.cn.fragment.MtgoxPersonInfoFragment;
import com.mt.gox.cn.fragment.MtgoxVolumeFragment;
import com.mt.gox.cn.fragment.SettingsFragment;
import com.mt.gox.cn.fragment.WebViewFragment;
...
public class MainActivity extends BaseActivity {
  ...
  private Uri currentUri = MtgoxPersonInfoFragment.PERSONINFO_URI;
...
protected void onResume() {
	if(!MTApplication.getInstance().isUnlock)
...
protected void onPause() {
	MTApplication.getInstance().isUnlock = false;
	super.onPause();
}
...
public void onActionsButtonClick(View view) {
  ...
  public void updateContent(Uri uri) {
    ...
    if (MtgoxPersonInfoFragment.PERSONINFO_URI.equals(uri)) {
      tag = MtgoxPersonInfoFragment.TAG;
      final Fragment foundFragment = fm.findFragmentByTag(tag);
      if (foundFragment != null) {
        fragment = foundFragment;
      } else {
        fragment = new MtgoxPersonInfoFragment();
      }
    } else if (MtgoxDepthFetchFragment.DEPTH_FETCH_URI.equals(uri)) {
        tag = MtgoxDepthFetchFragment.TAG;
        final Fragment foundFragment = fm.findFragmentByTag(tag);
        if (foundFragment != null) {
          fragment = foundFragment;
        } else {
          fragment = new MtgoxDepthFetchFragment();
        }
    } else if (MtgoxVolumeFragment.VOLUME_URI.equals(uri)) {
        tag = MtgoxVolumeFragment.TAG;
        final Fragment foundFragment = fm.findFragmentByTag(tag);
        if (foundFragment != null) {
          fragment = foundFragment;
        } else {
          fragment = new MtgoxVolumeFragment();
        }
    }  else if (BtcChinaFragment.BTCCHINA_URI.equals(uri)) {
        tag = BtcChinaFragment.TAG;
        final Fragment foundFragment = fm.findFragmentByTag(tag);
        if (foundFragment != null) {
          fragment = foundFragment;
        } else {
          fragment = new BtcChinaFragment();
        }
    }  else if (MtgoxWebFragment.MTGOX_URI.equals(uri)) {
        tag = MtgoxWebFragment.TAG;
        final Fragment foundFragment = fm.findFragmentByTag(tag);
        if (foundFragment != null) {
          fragment = foundFragment;
        } else {
          fragment = new MtgoxWebFragment();
        }
    }  else if (BtccDepthFetchFragment.DEPTH_FETCH_URI.equals(uri)) {
        tag = BtccDepthFetchFragment.TAG;
        final Fragment foundFragment = fm.findFragmentByTag(tag);
        if (foundFragment != null) {
          fragment = foundFragment;
        } else {
          fragment = new BtccDepthFetchFragment();
        }
    ...
    MTApplication.getInstance().mFragmentTag = tag;
